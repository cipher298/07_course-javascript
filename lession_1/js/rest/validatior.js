  ...
  function validate(rule, inputElement) {
    ...
    for (let i = 0; i < rulesOfEachElement.length; i++) {
      switch (inputElement.type) {
        case "radio":
        case "checkbox":
          errorMessage = rulesOfEachElement[i](
            // rulesOfEachElement[i] tương ứng với rule.test
    ...
    if (errorMessage) {
      getParrent(inputElement, option.formGroup).classList.add(
        option.errorMessageClass
      );
      errorElement.innerHTML = errorMessage;
    } else {
      getParrent(inputElement, option.formGroup).classList.remove(
        option.errorMessageClass
      );
...
Validator.isEmail = function (selector, message) {
  return {
    ...
    test: function (value) {
      ...
      return reget.test(String(value).toLowerCase())
        ? undefined
